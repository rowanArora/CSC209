...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
    ...
    for (int x = 0; x < height; ++x) {
        ...
        pixel_array[x] = malloc(sizeof(struct pixel) * width);
    ...
    for (int row = 0; row < height; ++row) {
        ...
        for (int col = 0; col < width; ++col) {
            ...
            fread(&pixel_array[row][col].blue, sizeof(unsigned char), 1, image);
            fread(&pixel_array[row][col].green, sizeof(unsigned char), 1, image);
            fread(&pixel_array[row][col].red, sizeof(unsigned char), 1, image);
