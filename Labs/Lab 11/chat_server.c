...
int read_from(int client_index, struct sockname *users) {
    ...
    if (users[client_index].username == NULL) {
        users[client_index].username = malloc(sizeof(char) * (BUF_SIZE + 1));
        strcpy(users[client_index].username, buf);
        return 0;
    }
    else {
        char *current_msg = malloc(2 * BUF_SIZE + 2);
        strcpy(current_msg, users[client_index].username);
        current_msg[strlen(users[client_index].username) - 1] = '\0';
        strcat(current_msg, ": \0");
        strcat(current_msg, buf);
        int r = write(fd, current_msg, strlen(current_msg));

        int index = 0;
        while (index < MAX_CONNECTIONS) {
            if (users[index].sock_fd != -1 && users[index].sock_fd != fd) {
                write(users[index].sock_fd, current_msg, strlen(current_msg));
            }
            index++;
        }

        if (num_read == 0 || r != strlen(current_msg)) {
